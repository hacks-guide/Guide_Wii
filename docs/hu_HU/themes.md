...
- NE FOLYTASD ezt az útmutatót, csak akkor, ha megfelelő brick védelmed van telepítve a [BootMii](hbc)-vel (csak Wii) és [Priiloader](priiloader)-rel.
...
3. Navigálj `System` -> `System Menu` -> `Your Region` opcióhoz, majd válaszd a Wii menü verziót amit futtatsz. Pipáld be a `Create decrypted contents (*.app)` opciót, vedd ki a pipát a `Keep encrypted contents` opciónál, majd kattints a `Start Download`-ra.

   ![](/images/desktop-apps/nusget/nusget-wiimenu-dl.png)
4. Ha befejezted a Wii menu fájlok letöltését, kattints mappára a `Start Download` mellett. Ez meg fogja nyitni a fájlkezelődet a NUSGet letöltési mappájába.

   ![](/images/desktop-apps/nusget/nusget-open-downloads.png)
5. Meg kell keresned és nyitnod benne a `0000000100000002` mappát.

   ![](/images/desktop-apps/nusget/nusget-folder-wiimenu.png)
6. Keresd meg a **legnagyobb .app fájlt** (általában ~6-7 Mb) a mappában.

   ![](/images/desktop-apps/nusget/nusget-wiimenu-output.png)
7. Hozz létre egy `themes` mappát az SD-kártya/USB-eszköz gyökerében.
8. Másold az.app fájlt a frissen létrehozott `themes` mappába. Ez az alap téma, amit használni fogsz az egyéni témád létrehozásához.
...
5. Pipáld be a `Create decrypted contents (*.app)` opciót, vedd ki a pipát a `Keep encrypted contents` opciónál, majd kattints a `Start Download`-ra.

   ![](/images/desktop-apps/nusget/nusget-vwiimenu-dl.png)

6. Ha befejezted a Wii menu fájlok letöltését, kattints mappára a `Start Download` mellett. Ez meg fogja nyitni a fájlkezelődet a NUSGet letöltési mappájába.

   ![](/images/desktop-apps/nusget/nusget-open-downloads.png)

7. Meg kell keresned és nyitnod benne a `0000000700000002` mappát.

   ![](/images/desktop-apps/nusget/nusget-folder-vwiimenu.png)

8. Keresd azt az .app fájlt, ami az alábbi táblázatból megfelel a Wii U-d régiójának:
...
9. Hozz létre egy `themes` mappát az SD-kártya/USB-eszköz gyökerében.

10. Másold az.app fájlt a frissen létrehozott `themes` mappába. Ez az alap téma, amit használni fogsz az egyéni témád létrehozásához.
