...
1. This will fill in information such as the keys, Boot 1/2, and your bad blocks automatically. Change *only* the Destination box to `./blankNand.bin`, which will output the NAND to the program folder with the name `blankNand.bin`. Then, press OK.
...
1. Once the menu has finished downloading, a pop-up will display showing the settings appended to the NAND. It will be autofilled with what was in your NAND, although the serial number may be incorrect. You can edit this if you would like, otherwise leave everything as-is and press OK.
